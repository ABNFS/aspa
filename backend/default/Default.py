...
class RepositoryDefault:
    ...
    async def get_all(self, db: Session, cls: ClassVar[Base]) -> list[Base]:
        result: list = []
...
class ServiceDefault:
    ...
    async def save(self, db: Session, data: DataModelDefault, max_deep: int = DEFAULT_DEEP):
        db_obj: Base
        new: bool = False
    ...
    async def get_all(self, db: Session, max_deep: int = DEFAULT_DEEP):
        return await convert_base_to_dict(await self.repository.get_all(db, self.database_class), max_deep=max_deep), \
               status.HTTP_200_OK

    async def get(self, db: Session, id: int, max_deep: int = DEFAULT_DEEP):
        item = await self.repository.get_by_id(db, self.database_class, id)
        return await convert_base_to_dict(item, max_deep=max_deep), \
               status.HTTP_200_OK if item else status.HTTP_404_NOT_FOUND

    async def delete(self, db: Session, id: int | dict):
        deleted = await self.repository.delete(db, self.database_class, id)
...
class ControllerDefault:
        ...
        if data:
            ...
            if isinstance(data, list):
                ...
                for item_data in data:
                    __item, _ = await service.save(__db, item_data, max_deep=1)
                    save_return_data.append(__item)
            else:
                __item, _status = await service.save(__db, data)
                save_return_data.append(__item)
        else:
    ...
    async def search(self, service: Optional[ServiceDefault] = None, name: Optional[str] = "", id: Optional[int] = -1,
        ...
        if id >= 0:
            ...
            _item, _status = await service.search(db, free_fields)
        else:
            _item, _status = await service.get_all(db)
    ...
    async def delete(self, service: Optional[ServiceDefault] = None, id: Optional[int] = -1,
        ...
        _deleted, _status = await service.delete(_db, id)
